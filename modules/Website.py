...
from modules.Manager import ListManager, WebsiteManager
...
def search_page(media_type, search_input, page_number):
    response = websiteManager.searchEntry(media_type, search_input, page_number)
    if response is -1:
        return redirect(url_for("not_found_404"), code=302)
    return render_template("media.html", response=response)
